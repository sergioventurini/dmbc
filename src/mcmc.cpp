	...
	while (niter < totiter){
		...
		for(int g = 0; g < G; g++){
			...
			for(int i = 0; i < n; i++){
				...
				loglik_rbmds(loglik_z_prop, Dm_g, z_prop, alpha[g], n, p, ng_tmp[g]);
				loglik_rbmds(loglik_z, Dm_g, z_g, alpha[g], n, p, ng_tmp[g]);
			...
			alpha_old = alpha[g];
			alpha_new = rnorm(alpha_old, gamma_alpha);

			loglik_rbmds(loglik_alpha_prop, Dm_g, z_g, alpha_new, n, p, ng_tmp[g]);
			loglik_rbmds(loglik_alpha, Dm_g, z_g, alpha_old, n, p, ng_tmp[g]);
			alpha_A = *loglik_alpha_prop - *loglik_alpha;
			alpha_B = dnorm(alpha_new, 0, sqrt(sigma2[g]), 1) - dnorm(alpha_old, 0, sqrt(sigma2[g]), 1);
		...
		for(int g = 0; g < G; g++){
			...
			lprior_alpha = dnorm(alpha[g], 0, sqrt(sigma2[g]), 1);
			sum_z2 = 0;
			for(int j = 0; j < p; j++){
				for(int i = 0; i < n; i++){
			...
			dinvgamma(lprior_eta, &eta[g], (hyper_eta_a[g] + n*p/2.0), (hyper_eta_b[g] + sum_z2/2.0), 1, 1);
			dinvgamma(lprior_sigma2, &sigma2[g], (hyper_sigma2_a + 1), (hyper_sigma2_b + alpha[g]*alpha[g]/2.0), 1, 1);
		...
		loglik_dmbc(&loglik[niter - 1], Dm, z, alpha, lambda, x, n, p, S, G);
